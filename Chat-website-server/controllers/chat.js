...
const getMyGroups = TryCatch(async (req,res,next) => {
    const chats = await Chat.find({creator:req.user}).populate("members", "name avatar");
...
const addMembers = TryCatch(async (req,res,next) => {
    ...
    if(!members || members.length <1) return next(new ErrorHandler('please provide member',400));
    ...
    .filter((i) => !chat.members.map(id => id.toString()).includes(i._id.toString()))
    .map((i) => i._id);
...
const removeMembers = TryCatch(async (req,res,next) => {
   ...
   if(chat.creator.toString() !== req.user.toString()) return next(new ErrorHandler('you are not allowed to remove members',403))
    ...
    if(chat.members.length<=3) return next(new ErrorHandler('Group must have least 3 members',400));
...
const renameGroup = TryCatch(async (req,res,next) => {
    ...
    return res.status(200).json({
        success:true,
        msg:'Group name changed succssfully'
    })
});
...
const deleteChat = TryCatch(async (req,res,next) => {
    ...
    //HERE WE HAVE TO DELETE ALL MESSAGESS AS WELL AS ATTACHMENTS AND FILES FROM CLOUDINARY
